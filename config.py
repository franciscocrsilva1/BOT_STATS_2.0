...
# Configuração de Logging (Copied from original stats.py)
logging.basicConfig(
...
CREDS_JSON = os.environ.get("GSPREAD_CREDS_JSON") # Adicionado para Render

# Mapeamento de Ligas (Copied from original stats.py)
LIGAS_MAP = {
...
def conectar_gsheets():
    """Tenta estabelecer a conexão com o Google Sheets e popula a variável global client."""
    global client

    if not CREDS_JSON:
        logging.error("❌ ERRO: GSPREAD_CREDS_JSON não encontrada. O Job Queue será desativado.")
        return
    ...
    try:
        ...
        os.remove(tmp_file_path) # Remove o arquivo temporário
