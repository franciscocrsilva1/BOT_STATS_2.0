# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.2.8 - NOVOS FILTROS DE GOLS DO TIME
# ===============================================================================
...
async def atualizar_planilhas(context: ContextTypes.DEFAULT_TYPE):
    ...
    # AJUSTE: Tratamento de erro explícito para abrir a planilha
    try: 
        sh = client.open_by_url(SHEET_URL)
    except Exception as e:
    ...
    for aba_code, aba_config in LIGAS_MAP.items():
        ...
        # AJUSTE: Logging do resultado da busca da API
        ...
        await asyncio.sleep(10) # Pausa para respeitar limite de rate da API (10s > 6s, seguro)

        if jogos_finished:
            try:
                exist = ws_past.get_all_records()
                # AJUSTE: Logging do número de linhas de histórico lidas
                ...
                if novas_linhas:
                    ...
                    # AJUSTE: Logging quando não há novos jogos
            ...
            # AJUSTE: Mensagem de erro mais útil em caso de falha no GSHEETS
            except Exception as e:
                logging.error(f"❌ Erro ao ler ou inserir dados na planilha {aba_past}: {e}. Verifique se os cabeçalhos das colunas (Mandante, Visitante, Gols Mandante, etc.) correspondem **exatamente** aos nomes no código.")
...
def calcular_estatisticas_time(time, aba, ultimos=None, casa_fora=None):
    ...
    d = {"time":time,"jogos_time":0,"jogos_casa":0,"jogos_fora":0,
         ...
         # NOVOS FILTROS DE GOLS DO TIME
         "marcou2_plus": 0, "marcou2_plus_casa": 0, "marcou2_plus_fora": 0,
         "sofreu2_plus": 0, "sofreu2_plus_casa": 0, "sofreu2_plus_fora": 0,
    ...
    for linha in linhas:
        ...
        if em_casa:
            ...
            marcados1T, sofridos1T = gm1, gv1
            marcados2T, sofridos2T = gm2, gv2
            ...
            marcados1T, sofridos1T = gv1, gm1
            marcados2T, sofridos2T = gv2, gm2
        ...
        # GAT (Gol em Ambos os Tempos - Total)
        gol_no_1t = total1 > 0
        ...
        # NOVOS FILTROS DE GOLS DO TIME (Marcou/Sofreu 2+ e Marcou/Sofreu Ambos Tempos)
        if marcados >= 2: d["marcou2_plus"] += 1
        if sofridos >= 2: d["sofreu2_plus"] += 1
        
        if marcados1T > 0 and marcados2T > 0: d["marcou_ambos_tempos"] += 1
        if sofridos1T > 0 and sofridos2T > 0: d["sofreu_ambos_tempos"] += 1
        ...
        # Split para os novos filtros
        if em_casa:
            if marcados >= 2: d["marcou2_plus_casa"] += 1
            if sofridos >= 2: d["sofreu2_plus_casa"] += 1
            if marcados1T > 0 and marcados2T > 0: d["marcou_ambos_tempos_casa"] += 1
            if sofridos1T > 0 and sofridos2T > 0: d["sofreu_ambos_tempos_casa"] += 1
        else:
            if marcados >= 2: d["marcou2_plus_fora"] += 1
            if sofridos >= 2: d["sofreu2_plus_fora"] += 1
            if marcados1T > 0 and marcados2T > 0: d["marcou_ambos_tempos_fora"] += 1
            if sofridos1T > 0 and sofridos2T > 0: d["sofreu_ambos_tempos_fora"] += 1
...
def formatar_estatisticas(d):
    ...
    return (f"📊 **Estatísticas - {escape_markdown(d['time'])}**\n"
            ...
            f"🥅 **G.A.T. (Gol em Ambos os Tempos - Total): {pct(d.get('g_a_t',0), jt)}** (C: {pct(d.get('g_a_t_casa',0), jc)} | F: {pct(d.get('g_a_t_fora',0), jf)})\n\n"
            
            # --- NOVOS FILTROS DE GOLS DO TIME ---
            f"🎯 **Metas de Gols do Time:**\n"
            f"⬆️ Marcou 2+ Gols: **{pct(d['marcou2_plus'], jt)}** (C: {pct(d.get('marcou2_plus_casa',0), jc)} | F: {pct(d.get('marcou2_plus_fora',0), jf)})\n"
            f"⬇️ Sofreu 2+ Gols: **{pct(d['sofreu2_plus'], jt)}** (C: {pct(d.get('sofreu2_plus_casa',0), jc)} | F: {pct(d.get('sofreu2_plus_fora',0), jf)})\n"
            f"🔥 Marcou Ambos Tempos: **{pct(d['marcou_ambos_tempos'], jt)}** (C: {pct(d.get('marcou_ambos_tempos_casa',0), jc)} | F: {pct(d.get('marcou_ambos_tempos_fora',0), jf)})\n"
            f"🥶 Sofreu Ambos Tempos: **{pct(d['sofreu_ambos_tempos'], jt)}** (C: {pct(d.get('sofreu_ambos_tempos_casa',0), jc)} | F: {pct(d.get('sofreu_ambos_tempos_fora',0), jf)})\n\n"
            # -------------------------------------
    ...
    if status == "FUTURE":
        ...
        for jogo in jogos_a_listar:
            try:
                ...
                # ✅ CORREÇÃO: Mesmo que o nome seja longo, garantimos a segurança do callback
                callback_data = f"JOGO|{aba_code}|{M_full}|{V_full}"
                
                if len(callback_data.encode('utf-8')) > 64:
                    # Se estourar, usa as primeiras 8 letras de cada nome para o callback, mas armazena o nome completo no user_data mais tarde.
                    M_short = M_full.split(' ')[0][:8]
                    V_short = V_full.split(' ')[0][:8]
                    M_safe_short = escape_markdown(M_short)
                    V_safe_short = escape_markdown(V_short)
                    # O callback ainda usa o nome completo, mas cortado, garantindo o limite
        ...
        for jogo in jogos_a_listar:
            ...
            # ✅ CORREÇÃO: Aplica a mesma lógica de encurtamento para LIVE, se necessário
            callback_data = f"JOGO|{aba_code}|{M_full}|{V_full}"
...
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str):
    """ Quarta tela: Menu para escolher o filtro de Estatísticas/Resultados. Usa context.user_data para armazenar nomes longos."""
    
    # ✅ CORREÇÃO: Armazena o nome COMPLETO dos times no user_data com uma chave curta
    # O uso do chat_id garante que cada chat tenha seu próprio estado de confronto.
    confronto_id = f"C{update.effective_chat.id}" 
    context.user_data[confronto_id] = {'mandante': mandante, 'visitante': visitante}
    
    mandante_display = mandante
    visitante_display = visitante
    
    title = f"🔎 **{aba_code}** | {mandante_display} vs {visitante_display}\n\n**Escolha o Filtro de Análise:**"
    
    keyboard = []
    
    for label, tipo, ultimos, cond_m, cond_v in CONFRONTO_FILTROS:
        # Callback ENCURTADO: TIPO|ULTIMOS|M_FILTRO|V_FILTRO|ABA|ID_CONFRONTO
        cond_m_str = str(cond_m) if cond_m else "None"
        cond_v_str = str(cond_v) if cond_v else "None"
        # O callback agora tem APENAS o ID do confronto no user_data
        callback_data = f"{tipo}|{ultimos}|{cond_m_str}|{cond_v_str}|{aba_code}|{confronto_id}"
        keyboard.append([InlineKeyboardButton(label, callback_data=callback_data)])

    # O botão de voltar leva de volta à listagem de jogos (FUTURE/LIVE)
    keyboard.append([InlineKeyboardButton(f"⬅️ Voltar para Jogos ({aba_code})", callback_data=f"STATUS|FUTURE|{aba_code}")])

    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Tenta apagar a mensagem anterior (a lista de jogos)
    try:
...
async def processar_confronto(
    ...
    aba_code: str, confronto_id: str
):
    """
    Calcula e exibe as estatísticas, enviando a resposta e o menu de ações
    em mensagens separadas para evitar que a resposta seja "destruída".
    """
    
    # ✅ CORREÇÃO: RECUPERA NOMES COMPLETOS DOS TIMES
    if confronto_id not in context.user_data:
        logging.error(f"Dados de confronto perdidos para ID: {confronto_id}")
        await update.callback_query.edit_message_text("❌ Erro: Dados do confronto expiraram. Tente novamente iniciando com /stats.", parse_mode='Markdown')
        return

    nomes_confronto = context.user_data[confronto_id]
    mandante = nomes_confronto['mandante']
    visitante = nomes_confronto['visitante']
    ...
    # 6. Definir o menu de ações adicionais (NOVO MENU DE AÇÕES)
    keyboard_acoes = [
        # Opção 1: Voltar para o menu de filtros do jogo atual (re-executa JOGO|ABA|MANDANTE|VISITANTE)
        [InlineKeyboardButton("⬅️ Voltar para Filtros", callback_data=f"JOGO|{aba_code}|{mandante}|{visitante}")],
        # Opção 2: Voltar para a listagem inicial de Ligas
        [InlineKeyboardButton("🏠 Início (/stats)", callback_data="VOLTAR_LIGA")],
    ]
    reply_markup_acoes = InlineKeyboardMarkup(keyboard_acoes)

    # 7. ENVIAR A MENSAGEM FINAL DE ESTATÍSTICAS
    try:
        # Envia a mensagem de estatísticas, que não será modificada.
        await update.effective_message.reply_text(
            msg_final,
            parse_mode='Markdown'
        )
        
        # 8. ENVIAR A MENSAGEM DE OPÇÕES SEPARADAMENTE
        await update.effective_message.reply_text(
            "Selecione uma ação:",
            reply_markup=reply_markup_acoes, 
            parse_mode='Markdown'
        )
        
        # 9. Limpar a mensagem anterior de "loading" ou de menu de filtros
        try:
            # Esta chamada apaga a mensagem de "⏳ Analisando..."
            await update.callback_query.delete_message()
        except Exception:
            pass # Se o delete falhar, é ok, a mensagem de "loading" ficará lá.

    except Exception as e:
        # Tratamento de erro se o envio da mensagem principal falhar (ex: mensagem muito longa)
        logging.error(f"Erro ao enviar mensagem final em processar_confronto: {e}")
        await update.effective_message.reply_text(
            "❌ Erro ao exibir estatísticas (Mensagem muito longa?). Tente reiniciar o fluxo.",
            reply_markup=reply_markup_acoes, 
            parse_mode='Markdown'
        )
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data[0] == "JOGO":
            ...
            mandante = data[2]
            visitante = data[3]
            # Usa o nome ENCURTADO/SEGURADO, que é seguro para a chamada do menu de ações
        ...
        if data[0] in ["STATS_FILTRO", "RESULTADOS_FILTRO"]:
            # Callback: TIPO|ULTIMOS|M_FILTRO|V_FILTRO|ABA|ID_CONFRONTO
            tipo = data[0]
        ...
        if "Message is not modified" in str(e):
            logging.info("Mensagem não modificada, ignorando BadRequest.")
             ...
             # ✅ TRATAMENTO ESPECÍFICO PARA ERRO DE LIMITE DE CALLBACK
            logging.error(f"Telegram BadRequest - Button_data_invalid: O callback excedeu 64 bytes. {e}", exc_info=True)
            try:
                 await update.callback_query.edit_message_text("❌ ERRO: O botão clicado contém dados muito longos. Tente reiniciar o fluxo com /stats.", parse_mode='Markdown')
            except:
                 await update.effective_message.reply_text("❌ ERRO: O botão clicado contém dados muito longos. Tente reiniciar o fluxo com /stats.", parse_mode='Markdown')
        else:
...
def main():
    ...
    # ✅ CORREÇÃO: Inicializa a classe JobQueue
    job_queue_instance = JobQueue() 
    
    # ✅ CORREÇÃO: Passa a instância do JobQueue para o ApplicationBuilder
