...
# Mapeamento de Ligas (Filtrado para BL1, BSA, DED, PL)
LIGAS_MAP = {
...
# =================================================================================
# 📈 FUNÇÕES DE CÁLCULO E FORMATAÇÃO (CORRIGIDAS)
# =================================================================================
def calcular_estatisticas_time(time, aba, ultimos=None, casa_fora=None):
    ...
    # Filtro Corrigido: Se casa_fora estiver definido, filtra especificamente a coluna correta
    if casa_fora == "casa": 
...
def listar_ultimos_jogos(time, aba, ultimos=None, casa_fora=None):
    try: 
        ...
        return f"⚠️ Erro ao ler dados da planilha para {escape_markdown(time)}."

    # Filtro Corrigido para Listagem de Resultados
    if casa_fora == "casa": 
        linhas = [l for l in linhas if l['Mandante'] == time]
    elif casa_fora == "fora": 
        linhas = [l for l in linhas if l['Visitante'] == time]
    ...
    if ultimos: 
        linhas = linhas[-ultimos:]
    if not linhas: 
        return f"Nenhum jogo encontrado para **{escape_markdown(time)}**."
