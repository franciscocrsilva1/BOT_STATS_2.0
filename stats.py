...
# ✅ COMANDO FORÇA UPDATE - IMPLEMENTAÇÃO CORRIGIDA
async def forcaupdate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    feedback_msg = await update.message.reply_text("⚡️ **Atualização Forçada Iniciada!** O processo de sincronização com o GSheets pode levar alguns segundos...", parse_mode='Markdown')
    
    try:
        # Chama a função assíncrona diretamente (corrigido: removido asyncio.to_thread)
        await atualizar_planilhas(context) 
        # Edita a mensagem para confirmar o sucesso
        await feedback_msg.edit_text("✅ Atualização forçada concluída! O cache e as abas de histórico foram sincronizados.")
    except Exception as e:
        logging.error(f"Erro ao forçar atualização: {e}")
        # Trata erros e edita a mensagem com feedback de erro
        error_text = f"❌ Erro na atualização: {escape_markdown(str(e))}\n\nConsulte os logs para mais detalhes."
        try:
            await feedback_msg.edit_text(error_text, parse_mode='Markdown')
        except:
