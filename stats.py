# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.4.0 - ATUALIZADO
# ===============================================================================
...
# Correção: Inclusão de Prorrogação e Pênaltis
...
def buscar_jogos_live(league_code):
    ...
    try:
        # Busca intervalo de 24h para garantir que jogos que viraram o dia apareçam
...
# ... (Funções de cálculo, Start, Stats e Callbacks idênticas ao original) ...
...
async def callback_query_handler(u, c):
    ...
    try:
        if d.startswith("c|"): await mostrar_menu_status_jogo(u, c, d.split('|')[1])
            ...
            kb.append([InlineKeyboardButton("⬅️ Voltar", callback_data=f"VOLTAR_LIGA_STATUS|{aba}")])
            await u.effective_message.reply_text(f"Filtros para {jogo['Mandante_Nome']} x {jogo['Visitante_Nome']}:", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
