...
async def mostrar_menu_status_jogo(update: Update, aba_code: str):
    ...
    await update.callback_query.edit_message_text(f"**{aba_code}** - Selecione:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
...
async def listar_jogos(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, status: str):
    ...
    if not jogos:
        await update.callback_query.answer("Nenhum jogo encontrado.", show_alert=True)
        return
    ...
    for idx, j in enumerate(jogos):
        ...
        if status == "LIVE": label = f"🔴 {j['Tempo_Jogo']} | {label} ({j['Placar_Mandante']}x{j['Placar_Visitante']})"
...
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, m: str, v: str):
    context.chat_data.update({'m': m, 'v': v
