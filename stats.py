# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.2.3 - MIGRADO PARA RENDER (WEBHOOK)
# ===============================================================================
...
# CORREÇÃO CRÍTICA: Adiciona 'context' para ser chamado corretamente pelo JobQueue.
...
# O Render fornece 'RENDER_EXTERNAL_URL' e 'PORT' automaticamente
HOST_URL = os.environ.get("RENDER_EXTERNAL_URL")
PORT = int(os.environ.get("PORT", 8443))
# BOT_TOKEN já foi pego lá em cima

# Inicializa o aplicativo do bot (agora global para ser acessado pelo Flask)
app = None
...
# =================================================================================
# 🚨 FUNÇÃO WEBHOOK CRÍTICA - CORREÇÃO APLICADA AQUI! 
# =================================================================================
...
def webhook():
    ...
    if request.get_json():
        try:
            # Converte o JSON recebido do Telegram em um objeto "Update"
            update = Update.de_json(request.get_json(), app.bot)
            
            # Método correto para passar a atualização para o loop assíncrono do bot
            app.post(update) 
            
        except Exception as e:
            # Em caso de erro, apenas loga e retorna 500 para o Telegram (sem crashar o Flask)
...
def main():
    global app  # Usa a variável global 'app'
    ...
    # === Job Queue (CORREÇÃO CRÍTICA: Não usa asyncio.run()) ===
    if client:
        job_queue: JobQueue = app.job_queue
        
        # FIX: Executa o pré-carregamento de cache como uma tarefa do Job Queue que roda 1x no início.
        # Isso evita o 'RuntimeError: asyncio.run() cannot be called...'
    ...
    # ===== MUDANÇA CRÍTICA: DE POLLING PARA WEBHOOK =====
    ...
    if not HOST_URL:
        ...
        # Fallback para polling (se rodar localmente sem a URL)
        app.run_polling()
        return
