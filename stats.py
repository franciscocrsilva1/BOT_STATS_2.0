# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.2.3 - CORREÇÃO FINAL WEBHOOK
# ===============================================================================
...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, Update, Bot # 🚨 Importei 'Bot'
...
# CORREÇÃO CRÍTICA (Asyncio.run): Adiciona 'context' para ser chamado corretamente pelo JobQueue.
...
async def listar_jogos(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, status: str):
    ...
    if status == "FUTURE":
        ...
        # Salva a lista de jogos no cache da conversa
        context.chat_data[cache_key] = jogos_a_listar

        keyboard = []
        # Adiciona enumerate(idx)
        for idx, jogo in enumerate(jogos_a_listar):
            try:
                ...
                try:
                    ...
                    data_local = data_utc - timedelta(hours=3) # Fuso -3
                ...
                # Callback usa o ÍNDICE (idx), não os nomes
...
async def mostrar_menu_acoes(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, mandante: str, visitante: str):
    ...
    SEMPRE envia o menu de filtros como uma NOVA MENSAGEM.
    """
    ...
    keyboard = []
    # Cria botões para Estatísticas e Resultados
    for idx, (label, tipo_filtro, ultimos, condicao_m, condicao_v) in enumerate(CONFRONTO_FILTROS):
        
        # Callback NÃO inclui mais os nomes, apenas o índice do FILTRO.
    ...
    # Responde ao callback para fechar o relógio de loading no botão clicado (JOGO|...)
    await update.callback_query.answer()


# =================================================================================
# ✅ FUNÇÃO: EXIBIÇÃO DAS ESTATÍSTICAS
# =================================================================================
async def exibir_estatisticas(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    ...
    Envia o resultado como NOVA MENSAGEM e REEXIBE o menu.
    """
    ...
    # 1. Responde com a ESTATÍSTICA como uma NOVA MENSAGEM
    await update.effective_message.reply_text(
        f"**Confronto:** {escape_markdown(mandante)} x {escape_markdown(visitante)}\n\n{texto_estatisticas}",
        parse_mode='Markdown'
    )
    
    # 2. Reexibe o menu de opções logo abaixo da estatística
...
# =================================================================================
# ✅ FUNÇÃO: EXIBIÇÃO DOS ÚLTIMOS RESULTADOS
# =================================================================================
async def exibir_ultimos_resultados(update: Update, context: ContextTypes.DEFAULT_TYPE, mandante: str, visitante: str, aba_code: str, filtro_idx: int):
    ...
    # 1. Responde com os RESULTADOS como uma NOVA MENSAGEM
    await update.effective_message.reply_text(
        f"**Confronto:** {escape_markdown(mandante)} x {escape_markdown(visitante)}\n\n{texto_final}",
        parse_mode='Markdown'
    )
    
    # 2. Reexibe o menu de opções logo abaixo dos resultados
    await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # 1. Escolha de Competição
        if data.startswith("c|"):
            _, aba_code = data.split('|')
            await mostrar_menu_status_jogo(update, context, aba_code)
            return
            
        # 2. Escolha de Status (Live/Future)
        if data.startswith("STATUS|"):
            _, status, aba_code = data.split('|')
            await listar_jogos(update, context, aba_code, status)
            return

        # 3. Seleção de Jogo (JOGO|ABA_CODE|STATUS|INDEX)
        if data.startswith("JOGO|"):
            _, aba_code, status, idx_str = data.split('|')
            idx = safe_int(idx_str)

            jogo = None
            # Chave de cache dinâmica
            jogos_key = f"{aba_code}_jogos_{status.lower()}" # e.g., 'BL1_jogos_future'
            
            if jogos_key in context.chat_data:
                jogos_list = context.chat_data.get(jogos_key, [])
                if 0 <= idx < len(jogos_list):
                    jogo = jogos_list[idx]

            if not jogo:
                await query.answer("❌ Erro: Jogo não encontrado no cache. Por favor, reinicie o menu com /stats.", show_alert=True)
                return

            # Nomes completos recuperados
            mandante = jogo['Mandante_Nome']
            visitante = jogo['Visitante_Nome']
            
            # Salva os nomes completos no chat_data para a PRÓXIMA etapa (Filtros)
            context.chat_data['current_mandante'] = mandante
            context.chat_data['current_visitante'] = visitante
            context.chat_data['current_aba_code'] = aba_code

            # Chama a função que mostra o menu de filtros
            await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
            return

        # 4. Filtro de Estatísticas (STATS_FILTRO|INDEX)
        if data.startswith("STATS_FILTRO|"):
            _, idx_str = data.split('|')
            filtro_idx = safe_int(idx_str)

            # Recupera os dados do contexto
            if 'current_mandante' not in context.chat_data or 'current_visitante' not in context.chat_data or 'current_aba_code' not in context.chat_data:
                 await query.answer("❌ Erro: Sessão expirada. Por favor, reinicie o menu com /stats.", show_alert=True)
                 return
                 
            mandante = context.chat_data['current_mandante']
            visitante = context.chat_data['current_visitante']
            aba_code = context.chat_data['current_aba_code']
            
            await exibir_estatisticas(update, context, mandante, visitante, aba_code, filtro_idx)
            return
            
        # 5. Filtro de Últimos Resultados (RESULTADOS_FILTRO|INDEX)
        ...
        try:
             await update.callback_query.edit_message_text(f"❌ Ocorreu um erro interno. Tente novamente iniciando com /stats.\nErro: {e}")
        except:
             await update.effective_message.reply_text(f"❌ Ocorreu um erro interno. Tente novamente iniciando com /stats.\nErro: {e}")
...
# =================================================================================
# 🚀 FUNÇÃO PRINCIPAL (MODIFICADA PARA WEBHOOK NO RENDER)
# =================================================================================

# Variáveis de ambiente para o Webhook
HOST_URL = os.environ.get("RENDER_EXTERNAL_URL")
PORT = int(os.environ.get("PORT", 8443))

# Inicializa o aplicativo do bot e o Bot separadamente (agora globais)
app = None
bot_instance = None # 🚨 Variável global para a instância do Bot

# Inicializa o servidor web Flask
server = Flask(__name__)

# Rota de "saúde" - Apenas para o Render saber que o app está vivo
@server.route("/")
def health_check():
    return "Bot de Estatísticas está vivo!", 200

# =================================================================================
# 🚨 FUNÇÃO WEBHOOK CRÍTICA - CORREÇÃO FINAL AQUI! 
# =================================================================================
@server.route(f"/{BOT_TOKEN}", methods=["POST"])
def webhook():
    global app
    global bot_instance # 🚨 Acesso à instância do Bot
    
    # Ponto de verificação de inicialização
    if not app or not bot_instance:
        logging.error("❌ O Bot não foi inicializado corretamente antes de receber o webhook.")
        return "Not Initialized", 503

    if request.get_json():
        try:
            # ✅ CORREÇÃO: Usa 'bot_instance' para converter o JSON, evitando o AttributeError
            update = Update.de_json(request.get_json(), bot_instance) 
            
            # Envia a atualização para o loop assíncrono do Application
            app.post(update) 
...
def main():
    ...
    # 3. Obtém a instância do Bot (essencial para o webhook)
    bot_instance = app_builder.bot
    ...
    # === Job Queue (FIX de asyncio.run() mantido) ===
    if client:
        job_queue: JobQueue = app.job_queue
        
        # FIX: Executa o pré-carregamento de cache como uma tarefa do Job Queue que roda 1x no início.
        job_queue.run_once(pre_carregar_cache_sheets, when=0, name="PreCarregarCache") 
        
        # Roda a atualização 1 hora (3600s) depois do início e repete a cada 1 hora
    ...
    if not HOST_URL:
        logging.error("Variável HOST_URL não encontrada. O bot não pode iniciar sem a URL externa (Render).")
        logging.warning("Rodando em modo POLLING localmente (NÃO FUNCIONARÁ NO RENDER)")
        app.run_polling()
        return

    # 1. Define a URL completa do Webhook
    webhook_url = f"{HOST_URL}/{BOT_TOKEN}"
    
    # 2. Informa ao Telegram para ONDE enviar as atualizações
    try:
        # Define o webhook usando a instância do bot
        # ✅ CORREÇÃO: set_webhook deve ser chamado com a instância do Bot
        asyncio.run(bot_instance.set_webhook( 
            url=webhook_url,
            allowed_updates=Update.ALL_TYPES
        ))
        logging.info(f"✅ Webhook configurado com sucesso para: {webhook_url}")
    except Exception as e:
        logging.error(f"❌ Erro ao configurar o webhook: {e}")
        sys.exit(1)

    # 3. Inicia o servidor Flask (que espera o Telegram enviar dados)
