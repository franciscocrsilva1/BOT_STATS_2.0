# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.2.4 - VERSÃO FINAL PARA DEPLOY NO RENDER
# ===============================================================================
...
def main():
    ...
    # ✅ CORREÇÃO FINAL: Usa .updater().job_queue(True) para configurar o JobQueue
    app = ApplicationBuilder().token(BOT_TOKEN).updater().job_queue(True).build()
    
    app.add_handler(CommandHandler("start", start_command))
    app.add_handler(CommandHandler("stats", listar_competicoes))
    app.add_handler(CallbackQueryHandler(callback_query_handler))
    
    # ✅ CORREÇÃO CRÍTICA: TRATAMENTO DE URL NULA 
    ...
    if client:
        ...
        # Deve ser chamado fora do JobQueue, mas antes do app.run_webhook
        # Usamos asyncio.run() porque nest_asyncio.apply() já foi chamado no topo do script
    ...
    # ✅ CORREÇÃO DE ERRO PTB[WEBHOOKS]
    # O PTB 20+ exige a instalação da dependência [webhooks]. Isso é corrigido no requirements.txt.
    app.run_webhook(
        listen="0.0.0.0",
