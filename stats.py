# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.3.2 - LIGAS EXPANDIDAS & SYNC OTIMIZADO
# ===============================================================================
...
# Mapeamento de Ligas (BSA, BL1 e PL + Novas Ligas: DED, PD, FL1, ELC, PPL, SA)
LIGAS_MAP = {
        ...
        try:
            ...
            if jogos_f:
                ...
                novas = [[j["Mandante"], j["Visitante"], j["Gols Mandante"], j["Gols Visitante"],
                          ...
                          j["Gols Visitante 2T"], j["Data"]] for j in jogos_f if (j["Mandante"], j["Visitante"], j["Data"]) not in keys_exist]
        ...
        try:
            ...
            if jogos_fut:
                linhas = [[m.get("homeTeam", {}).get("name"), m.get("awayTeam", {}).get("name"), 
                           m.get('utcDate'), m.get("matchday")] for m in jogos_fut]
                ws_future.append_rows(linhas, value_input_option='USER_ENTERED')
        except: pass
        await asyncio.sleep(2)

# =================================================================================
# 📈 FUNÇÕES DE CÁLCULO E FORMATAÇÃO (Lógica idêntica à V2.3.1)
# =================================================================================
def calcular_estatisticas_time(time, aba, ultimos=None, casa_fora=None):
    ...
    for l in linhas:
        ...
        d["jogos_casa" if em_casa else "jogos_fora"] += 1
        d["gols_marcados"] += m; d["gols_sofridos"] += s
        d["total_gols"] += (gm+gv)
        if em_casa: d["gols_marcados_casa"] += m; d["gols_sofridos_casa"] += s; d["total_gols_casa"] += (gm+gv)
        else: d["gols_marcados_fora"] += m; d["gols_sofridos_fora"] += s; d["total_gols_fora"] += (gm+gv)
        ...
        if (gm2+gv2)>0.5: d["over05_2T"] += 1; d["over05_2T_casa" if em_casa else "over05_2T_fora"] += 1
        if (gm2+gv2)>1.5: d["over15_2T"] += 1; d["over15_2T_casa" if em_casa else "over15_2T_fora"] += 1
...
# =================================================================================
# 🤖 HANDLERS DO BOT (BSA, BL1, PL, DED, PD, FL1, ELC, PPL, SA)
# =================================================================================
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 **Bot de Estatísticas V2.3.2**\nUse **/stats** para começar.", parse_mode='Markdown')
...
async def listar_competicoes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message: await update.message.reply_text("Escolha a Liga:", reply_markup=markup)
    else: await update.callback_query.edit_message_text("Escolha a Liga:", reply_markup=markup)
...
async def mostrar_menu_status_jogo(update: Update, aba_code: str):
    ...
    await update.callback_query.edit_message_text(f"Liga: {aba_code}", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def listar_jogos(update: Update, context: ContextTypes.DEFAULT_TYPE, aba_code: str, status: str):
    ...
    if status == "FUTURE":
        ...
        jogos = [j for j in jogos if datetime.strptime(j['Data_Hora'][:16], '%Y-%m-%dT%H:%M').replace(tzinfo=timezone.utc) > agora][:MAX_GAMES_LISTED]
    else:
        jogos = buscar_jogos_live(aba_code)

    if not jogos:
        await update.callback_query.edit_message_text("Nenhum jogo encontrado.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Voltar", callback_data=f"c|{aba_code}")]]))
        return

    context.chat_data[cache_key] = jogos
    keyboard = []
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = q.data
    if d == "VOLTAR_LIGA": await listar_competicoes(update, context)
    elif d.startswith("c|"): await mostrar_menu_status_jogo(update, d.split('|')[1])
        ...
        jogo = context.chat_data.get(f"{aba}_jogos_{status.lower()}")[int(idx)]
        ...
        await q.message.reply_text(f"Filtros: {jogo['Mandante_Nome']} x {jogo['Visitante_Nome']}", reply_markup=InlineKeyboardMarkup(keyboard))
    elif d.startswith("STATS_FILTRO|") or d.startswith("RESULTADOS_FILTRO|"):
        tipo, idx = d.split('|'); idx = int(idx)
        m, v, aba = context.chat_data['current_mandante'], context.chat_data['current_visitante'], context.chat_data['current_aba_code']
        if "STATS" in tipo:
            _, _, u, cm, cv = CONFRONTO_FILTROS[idx]
...
def main():
    ...
    if webhook:
        app.run_webhook(listen="0.0.0.0", port=port, url_path=BOT_TOKEN, webhook_url=f"{webhook}/{BOT_TOKEN}")
    else:
        app.run_polling()
