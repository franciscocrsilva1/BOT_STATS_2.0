# ===============================================================================
# 🏆 BOT DE ESTATÍSTICAS DE CONFRONTO V2.2.8 - MENSAGEM ÚNICA COM BOTÕES
# ===============================================================================
...
# =================================================================================
# 🏃 FLUXO FINAL: PROCESSA E EXIBE ESTATÍSTICAS (AJUSTADO PARA MENSAGEM ÚNICA)
# =================================================================================
async def processar_confronto(
    ...
    """
    Calcula e exibe as estatísticas, ANEXANDO os botões de ação à mesma mensagem.
    Usa fallback para duas mensagens se a única falhar (mensagem muito longa).
    """
    ...
    # Mantém o "loading" para dar feedback rápido
    await update.callback_query.edit_message_text(
    ...
    filtro_mandante = "casa" if mandante_filtro and mandante_filtro != "None" else None
    filtro_visitante = "fora" if visitante_filtro and visitante_filtro != "None" else None
    ...
    # 5. Formatar a mensagem final (texto)
    separador = "\n\n" + "—"*20 + "\n\n"
    msg_final = msg_mandante + separador + msg_visitante

    # 6. DEFINIR O MENU DIRETO (Volta para o menu de filtros + Início)
    keyboard_acoes = [
    ...
    # 7. TENTAR ENVIAR A MENSAGEM ÚNICA DE ESTATÍSTICAS COM OS BOTÕES ANEXADOS
    try:
        # A mensagem de "loading" é a que está na tela agora.
        # Vamos EDITAR a mensagem de loading, trocando o texto pelo resultado e anexando os botões.
        await update.callback_query.edit_message_text(
            msg_final,
            reply_markup=reply_markup_acoes, # ANEXA OS BOTÕES
            parse_mode='Markdown'
        )
        
    except BadRequest as e:
        # Este erro acontece se: 
        # a) A mensagem não é modificada (já está tratada no handler geral)
        # b) A mensagem é muito longa para ser editada (limite ~4096 caracteres)
        logging.warning(f"Falha ao editar mensagem de estatísticas (muito longa?). Tentando 2 mensagens. Erro: {e}")
        
        # Tenta apagar a mensagem anterior de "loading" antes de enviar a nova
        try:
            await update.callback_query.delete_message()
        except Exception:
            pass 
            
        # Fallback 1: Envia as estatísticas sem botões
        await update.effective_message.reply_text(msg_final, parse_mode='Markdown')
        
        # Fallback 2: Envia os botões de ação em uma SEGUNDA mensagem
        await update.effective_message.reply_text(
            "👇 Menu de Ações (Voltar/Início) 👇",
            reply_markup=reply_markup_acoes, 
            parse_mode='Markdown'
        )
    except Exception as e:
        logging.error(f"Erro inesperado ao exibir estatísticas: {e}")
        await update.effective_message.reply_text("❌ Ocorreu um erro ao exibir estatísticas. Tente reiniciar o fluxo.", parse_mode='Markdown')

# =================================================================================
# 🔄 HANDLERS GERAIS E DISPATCHER
# =================================================================================
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Manipula todas as interações de botões inline."""
    query = update.callback_query
    await query.answer() # Fecha o relógio de carregamento
    ...
    try:
        ...
        if acao == "STATUS":
            status, aba_code = partes[1], partes[2]
            await listar_jogos(update, context, aba_code, status)
            return

        if acao == "JOGO":
            # JOGO|ABA|MANDANTE|VISITANTE
            aba_code, mandante, visitante = partes[1], partes[2], partes[3]
            await mostrar_menu_acoes(update, context, aba_code, mandante, visitante)
            return

        if acao in ["STATS_FILTRO", "RESULTADOS_FILTRO"]:
            # TIPO|ULTIMOS|M_FILTRO|V_FILTRO|ABA|ID_CONFRONTO
            _, ultimos_str, m_filtro, v_filtro, aba_code, confronto_id = partes
            ultimos = int(ultimos_str) if ultimos_str.isdigit() else ULTIMOS
            
            # Chama a função principal de processamento
            await processar_confronto(
                update, context, acao, ultimos, 
                m_filtro, v_filtro, aba_code, confronto_id
            )
            return

    except Exception as e:
        logging.error(f"Erro no callback_handler para data: {data}. Erro: {e}")
        try:
            await query.edit_message_text(f"❌ Ocorreu um erro. Tente novamente. Detalhe: {e}", parse_mode='Markdown')
        except:
             await query.message.reply_text(f"❌ Ocorreu um erro. Tente novamente. Detalhe: {e}", parse_mode='Markdown')

async def job_error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Manipulador de erros do JobQueue."""
    logging.error("Exceção no job (JobQueue):", exc_info=context.error)

def main():
    """Função principal que inicia o Bot."""
    if BOT_TOKEN == "SEU_TOKEN_AQUI":
        print("⚠️ ERRO: O token do bot não foi configurado. Por favor, configure a variável de ambiente BOT_TOKEN.")
        sys.exit(1)
    ...
    # Adicionar o job periódico (Atualização de planilhas)
    # Roda a cada 60 minutos (3600 segundos)
    job_queue.run_repeating(atualizar_planilhas, interval=3600, first=0, name='atualizar_planilhas')
    job_queue.set_error_handlers(job_error_handler)

    # Iniciar pré-carregamento do cache (executa uma vez no início)
    asyncio.run(pre_carregar_cache_sheets())

    logging.info("🚀 Bot iniciado e JobQueue configurado. Rodando em modo webhook.")
    
    # Uso recomendado para ambientes de produção (Render)
    PORT = int(os.environ.get('PORT', '8443'))
    WEBHOOK_URL = os.environ.get('WEBHOOK_URL', 'SEU_WEBHOOK_URL_AQUI')
    
    # Se o WEBHOOK_URL não for o default, inicia o webhook
    if WEBHOOK_URL != 'SEU_WEBHOOK_URL_AQUI':
        ...
        # Modo de polling, apenas para testes locais
        logging.warning("⚠️ Iniciando em modo POLING (para testes). Configure WEBHOOK_URL para produção.")
